...
namespace Empire_Rewritten.UI
{
    public class FlexRect
    {
        ...
        private float VerticalMargin, HorizontalMargin;
        ...
        public FlexRect CenterMargin(float marginX, float marginY, string name = "")
        {
            FlexRect fr = new FlexRect(name)
            {
                AnchorType = AnchorType.Center,
            ...
            foreach (FlexRect child in Children)
            {
                ...
                switch (child.AnchorType)
                {
                    ...
                    case AnchorType.Center:
                        float minWidth = Mathf.Max(child.MinimumSize.x / child.HorizontalRatio, child.HorizontalMargin * 2);
                        float minHeight = Mathf.Max(child.MinimumSize.y / child.VerticalRatio, child.VerticalMargin * 2);
                        minChildSize = new Vector2(minWidth, minHeight);
                        break;
                }
        ...
        public Dictionary<string, Rect> Resolve(Rect rect)
        {
            ...
            foreach (FlexRect child in Children)
            {
                ...
                switch (child.AnchorType)
                {
                    ...
                    case AnchorType.Center:
                        float newHeight = rect.height * child.VerticalRatio - child.VerticalMargin * 2;
                        float newWidth = rect.width * child.HorizontalRatio - child.HorizontalMargin * 2;
        ...
        public void MergeChildren()
        {
            Children.Sort((x, y) => x.AnchorType != y.AnchorType ? x.AnchorType.CompareTo(y.AnchorType) :
                ...
                x.HorizontalMargin != y.HorizontalMargin ? x.HorizontalMargin.CompareTo(y.HorizontalMargin) :
                x.VerticalMargin != y.VerticalMargin ? x.VerticalMargin.CompareTo(y.VerticalMargin) :
                        ...
                        switch (last.AnchorType)
                        {
                            ...
                            case AnchorType.Center:
                                if (last.VerticalRatio == child.VerticalRatio && last.HorizontalRatio == child.HorizontalRatio
                                    && last.HorizontalMargin == child.HorizontalMargin && last.VerticalMargin == child.VerticalMargin) merged = true;
                                break;
                            case AnchorType.Grid:
                                if (last.VerticalRatio == child.VerticalRatio && last.HorizontalRatio == child.HorizontalRatio
